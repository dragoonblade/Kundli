"""Remedial suggestions for doshas and afflicted planets."""

DOSHA_REMEDIES = {
    "Manglik Dosha": [
        "Chant Mangal (Mars) mantra: 'Om Kraam Kreem Kraum Sah Bhaumaya Namah' — 108 times on Tuesdays",
        "Wear a Red Coral (Moonga) gemstone after consulting an astrologer",
        "Fast on Tuesdays and donate red lentils (masoor dal)",
        "Visit Hanuman temple on Tuesdays and offer sindoor",
        "Manglik Dosha reduces significantly after age 28",
    ],
    "Kalsarpa Dosha": [
        "Perform Kalsarpa Shanti Puja at Trimbakeshwar or Kalahasti temple",
        "Chant Rahu mantra: 'Om Raam Rahave Namah' — 108 times on Saturdays",
        "Keep a silver snake idol at home and worship on Nag Panchami",
        "Donate black sesame seeds and dark cloth on Saturdays",
        "Feed birds daily — especially crows",
    ],
    "Sade Sati": [
        "Chant Shani mantra: 'Om Sham Shanaischaraya Namah' — 108 times on Saturdays",
        "Wear a Blue Sapphire (Neelam) or Amethyst only after expert consultation",
        "Light a sesame oil lamp under a Peepal tree on Saturdays",
        "Donate black urad dal, mustard oil, and iron items on Saturdays",
        "Serve and respect elders and workers — Saturn rewards discipline and humility",
    ],
}

PLANET_REMEDIES = {
    "Surya": {
        "gemstone": "Ruby (Manik)",
        "mantra": "Om Hraam Hreem Hraum Sah Suryaya Namah",
        "day": "Sunday",
        "donation": "Wheat, jaggery, copper",
        "practice": "Offer water to the rising Sun every morning (Surya Arghya)",
    },
    "Chandra": {
        "gemstone": "Pearl (Moti)",
        "mantra": "Om Shraam Shreem Shraum Sah Chandraya Namah",
        "day": "Monday",
        "donation": "Rice, white cloth, silver",
        "practice": "Keep a fast on Mondays and worship Lord Shiva",
    },
    "Mangal": {
        "gemstone": "Red Coral (Moonga)",
        "mantra": "Om Kraam Kreem Kraum Sah Bhaumaya Namah",
        "day": "Tuesday",
        "donation": "Red lentils, jaggery, copper",
        "practice": "Recite Hanuman Chalisa on Tuesdays",
    },
    "Budh": {
        "gemstone": "Emerald (Panna)",
        "mantra": "Om Braam Breem Braum Sah Budhaya Namah",
        "day": "Wednesday",
        "donation": "Green moong dal, green cloth",
        "practice": "Feed green vegetables to cows on Wednesdays",
    },
    "Guru": {
        "gemstone": "Yellow Sapphire (Pukhraj)",
        "mantra": "Om Graam Greem Graum Sah Gurave Namah",
        "day": "Thursday",
        "donation": "Yellow cloth, turmeric, chana dal, bananas",
        "practice": "Respect teachers and elders, worship Lord Vishnu on Thursdays",
    },
    "Shukra": {
        "gemstone": "Diamond (Heera) or White Sapphire",
        "mantra": "Om Draam Dreem Draum Sah Shukraya Namah",
        "day": "Friday",
        "donation": "White rice, white cloth, sugar, ghee",
        "practice": "Worship Goddess Lakshmi on Fridays",
    },
    "Shani": {
        "gemstone": "Blue Sapphire (Neelam) — only after expert consultation",
        "mantra": "Om Sham Shanaischaraya Namah",
        "day": "Saturday",
        "donation": "Black sesame, mustard oil, iron items, dark cloth",
        "practice": "Serve the underprivileged, light sesame oil lamp on Saturdays",
    },
    "Rahu": {
        "gemstone": "Hessonite Garnet (Gomed)",
        "mantra": "Om Raam Rahave Namah",
        "day": "Saturday",
        "donation": "Black sesame, coconut, blue cloth",
        "practice": "Feed birds, especially crows. Avoid intoxicants.",
    },
    "Ketu": {
        "gemstone": "Cat's Eye (Lehsunia)",
        "mantra": "Om Sraam Sreem Sraum Sah Ketave Namah",
        "day": "Tuesday/Saturday",
        "donation": "Blankets to the needy, sesame seeds",
        "practice": "Worship Lord Ganesha, practice meditation and detachment",
    },
}
